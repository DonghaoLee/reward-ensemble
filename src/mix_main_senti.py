...
wandb.init(
    ...
    name = 'training LoRA ensemble with group mix, senti, preference based'
)
...
reward_model.load_state_dict(torch.load('ckpt/mix_reward_model_senti_epoch_1.ckpt'))
...
for epoch in range(2, 3): # 2 epochs
    ...
    torch.save(reward_model.state_dict(), 'ckpt/mix_reward_model_senti_epoch_' + str(epoch) + '.ckpt')
