...
#    name = 'training LoRA ensemble with group mix, ratio 0.5, preference based'
#)
...
for epoch in range(2): # 2 epochs
    ...
    for i in range(len(dataset['text']) // batch):
        ...
        #})
        loss.backward()
        print(reward_model.weight.grad)
        ...
        for k in range(10):
            ...
            print(w[k])
            print(p)
            print(flag_list - base_prob)
            print(w[k] * p / (flag_list - base_prob))
            loss = torch.mean(w[k] * p / (flag_list - base_prob))
            loss.backward()
            for name, para in reward_model.named_parameters():
                if 'rwtransformer.decoder.layers.15.self_attn.out_proj.lora_left_weight' == name:
                    print(para.size())
                    print(torch.sum(torch.abs(para.grad), dim = (1, 2)))
            if k == 3:
                break
        break
        optimizer.step()
